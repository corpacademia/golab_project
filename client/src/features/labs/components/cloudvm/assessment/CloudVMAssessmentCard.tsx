...
const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, assessment, onSave }) => {
      ...
      <div className="bg-dark-200 rounded-lg w-full max-w-md p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">
            <GradientText>Edit Assessment</GradientText>
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2 bg-dark-400/50 border border-primary-500/20 rounded-lg
                         text-gray-300 focus:border-primary-500/40 focus:outline-none h-32"
                required
              />
            </div>
          </div>
...
export const CloudVMAssessmentCard: React.FC<CloudVMAssessmentProps> = ({ assessment }) => {
  ...
  const handleEditSave = async (data: any) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/v1/updateAssessment/${assessment.assessment_id}`, {
        ...data,
        admin_id: admin.id
      });
